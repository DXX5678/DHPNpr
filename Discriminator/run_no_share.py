...
def eval_acc_epoch(args, eval_data, eval_examples, model, loss_fn):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval acc"):
        ...
        sequence_1_ids, sequence_2_ids, labels = batch

        with torch.no_grad():
            outputs = model(sequence_1_ids, sequence_2_ids)
...
def main():
    ...
    if args.do_train:
        ...
        train_examples, train_data = load_and_cache_gen_data(args, args.train_dir, args.patch_train_dir, pool, tokenizer,
                                                             'train', mode="train")
        if args.local_rank == -1:
