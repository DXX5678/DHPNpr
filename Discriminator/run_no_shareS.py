...
def eval_acc_epoch(args, eval_data, eval_examples, model, loss_fn):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval acc"):
        ...
        with torch.no_grad():
            outputs = model(sequence_1_ids, sequence_2_ids, sequence_1_mask, sequence_2_mask)
...
def main():
    ...
    log_file = open(os.path.join(args.log_file_dir, 'Discriminator_No_ShareS_' + args.model_type + '.log'), 'a+')

    if args.do_train:
        # Prepare training data loader
        train_examples, train_data = load_and_cache_gen_data(args, args.train_dir, args.patch_train_dir, pool,
                                                             tokenizer, 'train', mode="train", no_share=False)
        if args.local_rank == -1:
            ...
            if args.do_eval and (int(cur_epoch) % 1 == 0):
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_examples, eval_data = load_and_cache_gen_data(args, args.dev_dir, args.patch_valid_dir, pool,
                                                                       tokenizer, 'dev', mode='dev', no_share=False)
