...
def eval_acc_epoch(args, eval_data, eval_examples, codet5, model, loss_fn):
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader), desc="Eval acc"):
        ...
        with torch.no_grad():
            sequence_1_output = codet5.encoder(sequence_1_ids, attention_mask=sequence_1_mask)
            sequence_2_output = codet5.encoder(sequence_2_ids, attention_mask=sequence_2_mask)
...
def main():
    ...
    if args.load_model_path is not None:
        ...
        codet5.load_state_dict(torch.load(args.load_model_path))
    ...
    if args.do_train:
        ...
        for cur_epoch in range(int(args.num_train_epochs)):
            ...
            if args.do_eval and (int(cur_epoch) % 1 == 0):
                ...
                eval_loss, eval_acc = eval_acc_epoch(args, eval_data, eval_examples, codet5, model, loss_fn)
